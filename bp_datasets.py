...
def compute_binding_pocket_features(pocket_featurizer, ligand_featurizer, pdb_subdir, pdb_code, threshold=.3):
    ...
    active_site_box, active_site_atoms, active_site_coords = (
        binding_pocket.extract_active_site(
    ...
    for ind, pocket in enumerate(pockets):
        overlap = binding_pocket.compute_overlap(pocket_atoms, active_site_box, pocket)
...
def featurize_pdbbind_pockets(data_dir=None, subset="core"):
    ...
    for ind, pdb_code in enumerate(ids):
        ...
        if not os.path.exists(pdb_subdir):
            ...
            continue
        features, labels = compute_binding_pocket_features(pocket_featurizer, ligand_featurizer, pdb_subdir, pdb_code)
        if features is None:
